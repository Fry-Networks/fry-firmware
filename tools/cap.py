# Bounded serial capture (PROTOCOL.md section 8).
#   py -3 tools/cap.py --port COMx --seconds N --out FILE [--reset] [--baud 115200]
#   py -3 tools/cap.py --proof-noreset          # 10s, assert opening does NOT reset the board
# Opens with DTR/RTS de-asserted so opening the port does not reset the board. --reset explicitly
# pulses a run-mode reset. Timestamps every line (ms since start), tees stdout + file, exits on
# its own after N seconds. NEVER an infinite loop. --port is optional — omit to auto-resolve by
# USB VID:PID (see _serialutil.py).
import sys, os, time, argparse
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import _serialutil as su

BOOT_MARKERS = ("FRY boot v", "[boot]", "chip=")
...
def main():
    ...
    if a.proof_noreset:
        ser = su.open_with_retry(a.port, a.baud, do_reset=False)
    ...
    ser = su.open_with_retry(a.port, a.baud, do_reset=a.reset)
    if a.reset:
